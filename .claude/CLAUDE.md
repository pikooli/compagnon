...
- Optional: `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` (enables Google Maps embed in event detail view; free Maps Embed API, restrict key by HTTP referrer)

# Interactive UI Commands
- **Pattern**: brain tools auto-push `UICommand` entries to `ctx.uiCommands` during execution; no separate "display" tool needed
- `app/types/ui-commands.ts` — shared types: `UICommand`, `DisplayCalendarCommand`, `UpdateCalendarEventCommand`, `AddCalendarEventCommand`, `RemoveCalendarEventCommand`, `FocusCalendarEventCommand`, `UnfocusCalendarEventCommand`, `CalendarEventData`, `EmailData`, `DisplayEmailsCommand`, `FocusEmailCommand`, `UnfocusEmailCommand`, `RemoveEmailCommand`, `DisplayDynamicCommand`, `DisplayDynamicData`, `KnownUICommand` union
...
- **Dynamic UI pattern**: `display_dynamic` commands replace older ones (same as calendar/email). Brain generates HTML via `show_ui` tool, sanitized client-side with DOMPurify. Display-only, no interactivity.
- `app/lib/brain/tools/show-ui.ts` — brain tool factory: `createShowUITool(ctx)` pushes `DisplayDynamicCommand`
- `app/components/interactive/DynamicDisplay.tsx` — client component: renders DOMPurify-sanitized HTML with dark theme, elderly-friendly 18px+ text
- `app/components/interactive/InteractivePanel.tsx` — reads `UICommandContext`, switch-renders by command `type` (`display_calendar`, `display_emails`, `display_dynamic`), `AnimatePresence` for animations
...
- `app/components/interactive/MeetingDetailView.tsx` — full event detail view: title, full date, time range, location + Google Maps embed (via `MapEmbed`), full description (no truncation), all attendees with response status icons (accepted=green, declined=red, tentative=yellow, no response=gray); back button returns to list
- `app/components/interactive/MapEmbed.tsx` — Google Maps Embed API iframe; renders when `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` is set and `address` prop is truthy; graceful no-op otherwise
